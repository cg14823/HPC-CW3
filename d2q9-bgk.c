...
int main(int argc, char* argv[])
{
  ...
  int size = (params.nx*params.ny)/16;
...
int reduce (t_ocl ocl, const t_param params, int tt, int size){
  ...
  err = clSetKernelArg(ocl.reduce, 2, sizeof(cl_float)*16,NULL);
  checkError(err, "setting reduce arg 2", __LINE__);
  err = clSetKernelArg(ocl.reduce, 3, sizeof(cl_int)*16, NULL);
               ...
               int** obstacles_ptr, float** av_vels_ptr, t_ocl *ocl)
{
  ...
  *av_vels_ptr = (float*)malloc(sizeof(float) * params->maxIters);
  ...
  ocl->results_reduce_u = clCreateBuffer(
    ocl->context, CL_MEM_READ_WRITE,
    sizeof(cl_float) *((params->nx*params->ny)/16), NULL, &err);
  checkError(err, "creating cells buffer", __LINE__);

  ocl->results_reduce_cells = clCreateBuffer(
    ocl->context, CL_MEM_READ_WRITE,
    sizeof(cl_int)*((params->nx*params->ny)/16), NULL, &err);
  checkError(err, "creating cells buffer", __LINE__);
